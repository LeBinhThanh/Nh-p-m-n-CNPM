...
public class BuyController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int iid = fdb.getInvoiceId()+1;
        fdb.insert_invoice(iid, date, username);
        
        Invoice invoice = (Invoice)request.getSession().getAttribute("invoice");
        for (InvoiceLine line : invoice.getLines()){
            String code = line.getFlower().getCode();
            float quantity = line.getQuantity();
            float price = line.getFlower().getPrice();
            fdb.insert_invoiceLine(iid,code, quantity, price);
        }
